...
public class TVEpisodesScene extends Scene {
    ...
    private final String SCENE_NAME = "TV EPISODES";
    ...
    public ArrayList<String> createMenuList() {
        ...
        if (new File("TV Shows/" + show.getName() + "/Season " + (seasonNumber + 1) + "/episode-list.json").exists()) {
            episodeList = parser.parseEpisodeList("TV Shows/" + show.getName() + "/Season " + (seasonNumber + 1) + "/episode-list.json");

            for (int x = 0; x < episodeList.episodes.size(); x++) {
                String epTitle = episodeList.episodes.get(x).getName();
                menuList.add(epTitle);
            }
        } else {
            menuList.add("No Episodes Yet!");
        }
    ...
    public void updateInformationLabels(int linkNum) {
        ...
        infoPanel.getStarRating().updateRating(episodeList.episodes.get(linkNum).getVoteAverage());
        infoPanel.getHUD().updateHUD(show.getName(),
                "Seasons: " + ScraperUtils.getNumberOfSeasons(show));
        infoPanel.getOverview().setText(episodeList.episodes.get(linkNum).getOverview());
    }
